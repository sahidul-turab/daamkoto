...
| **2026-08-03** | Product cutouts finally generated by CI, not by hand |
...
## 14. 2026-08-03 — Cutouts were never generated in CI

**Problem:** `run_pipeline.py` calls `scripts/remove_backgrounds.py` after every
load, so cutouts were supposed to be automatic. They never ran in CI once. The
scrape jobs in `daily-scrape.yml` install `playwright rapidfuzz psycopg2-binary
python-dotenv` — not `rembg` — so every category logged `ModuleNotFoundError`
and continued. The call is deliberately `fatal=False` so a cutout hiccup cannot
block a price update, which is right, and is exactly why nobody noticed: all
27,473 existing cutouts had been produced by hand on the dev machine.

**The obvious fix would have made it worse.** Adding `pip install rembg` to the
scrape jobs looks like a one-line change. Those jobs hold no R2 credentials, so
`R2_ENABLED` is false and the script writes cutouts to a local `/media` path on
an ephemeral runner, then records those paths in `image_cutouts`. The table is
the memory of what has been processed, so all 2,644 pending images would have
been marked done, pointing at files that never existed, and never retried.

**Change:** the cutout pass moved to the `report` job, which already holds the
R2 secrets, running after every category has loaded and before the bootstrap
snapshots publish so those snapshots carry the new image URLs. Scrape jobs now
pass `--no-cutouts` explicitly instead of attempting and failing.

**A second trap found on the way:** `R2_PUBLIC_BASE` was not among the
repository secrets. With credentials present but no public base,
`cutout_path` becomes a relative `/cutouts/<hash>` — same permanent breakage,
different cause. `remove_backgrounds.py` now refuses to start in that state
rather than poisoning the table, and the secret is set.

*Why it matters:* two of the three defects here shared one shape — a failure
