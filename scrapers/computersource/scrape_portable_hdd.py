"""Portable HDD scraper for ComputerSource — URL: /portable-hdd"""
...
START_URL = f"{BASE_URL}/portable-hdd"
PAGE_DELAY = 2.5
...
async def scrape_page(page, url: str) -> list[dict]:
    ...
    try: await page.wait_for_selector(".product", timeout=15_000)
    ...
    for card in cards:
        ...
        if not name_el: name_el = await card.query_selector(".name a")
        ...
        if not price_el: price_el = await card.query_selector(".price-wrap .price")
        if not price_el: price_el = await card.query_selector(".price")
        ...
        products.append({"name": name, "price_bdt": price, "in_stock": in_stock,
            "stock_status": stock_status, "product_url": product_url, "inline_specs": {},
            "source": "ComputerSource", "pc_bundle_only": pc_bundle_only, "scraped_at": scraped_at})
    return products
...
async def main(save: bool = False):
    ...
    if save:
        ...
        out_path = out_dir / f"computersource_portable_hdd_{ts}.json"
