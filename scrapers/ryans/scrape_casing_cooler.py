"""
Ryans Casing Cooler (case fans) scraper
URL: https://www.ryans.com/category/desktop-component-casing-fan

Uses fresh context per page to bypass Cloudflare JS challenge.

Usage:
  python scrapers/ryans/scrape_casing_cooler.py           # print only
  python scrapers/ryans/scrape_casing_cooler.py --save    # print + save JSON to data/raw/
"""
...
CATEGORY_URL = f"{BASE_URL}/category/desktop-component-casing-fan"
PAGE_DELAY = 2.5
...
async def scrape_page(page, url: str) -> list[dict]:
    ...
    try:
        ...
        print(f"  [warn] No cards found on {url}")
        return []
...
async def main(save: bool = False):
    ...
    if all_products:
        priced = [p for p in all_products if p["price_bdt"]]
        print(f"With price: {len(priced)}  |  In stock: {sum(1 for p in priced if p['in_stock'])}")

    if save and all_products:
        out_dir = Path("data/raw")
        out_dir.mkdir(parents=True, exist_ok=True)
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        out_path = out_dir / f"ryans_casing_cooler_{timestamp}.json"
