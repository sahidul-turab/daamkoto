"""Ryans Laptop RAM scraper — CF bypass, fresh context per page."""
...
CATEGORY_URL = f"{BASE_URL}/category/laptop-component-laptop-ram"
PAGE_DELAY   = 2.5
...
async def scrape_page(page, url: str) -> list[dict]:
    ...
    for card in cards:
        ...
        products.append({"name": name, "price_bdt": price,
            ...
            "product_url": product_url, "product_code_inv": item.get("product_code_inv"),
            "inline_specs": inline_specs, "source": "Ryans", "pc_bundle_only": False,
...
async def main(save: bool = False):
    ...
    if save:
        ...
        out_path = out_dir / f"ryans_laptop_ram_{ts}.json"
