"""
Ryans Processor scraper
Extracts: name, price (BDT), stock, product URL, inline specs from ryans.com

Same data-item JSON + CF-bypass pattern as the RAM/GPU scrapers.
Only CATEGORY_URL and output filename differ.

Usage:
  python scrapers/ryans/scrape_processor.py           # print only
  python scrapers/ryans/scrape_processor.py --save    # save JSON to data/raw/
"""
...
CATEGORY_URL = f"{BASE_URL}/category/desktop-component-processor"
PAGE_DELAY = 2.5
...
def save_results(products: list[dict]) -> Path:
    ...
    out_path = out_dir / f"ryans_processor_{timestamp}.json"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Scrape Ryans Processor listings")
