"""Portable HDD scraper for TrustTech — URL: /categories/portable-hdd"""
...
START_URL = f"{BASE_URL}/categories/portable-hdd"
PAGE_DELAY = 2.5
...
async def scrape_page(page, url: str) -> list[dict]:
    ...
    try: await page.wait_for_selector(".product-card", timeout=15_000)
    ...
    for card in cards:
        ...
        products.append({"name": name, "price_bdt": price, "in_stock": in_stock,
            ...
            "source": "TrustTech", "pc_bundle_only": pc_bundle_only, "scraped_at": scraped_at})
    return products
...
async def main(save: bool = False):
    ...
    if save:
        ...
        out_path = out_dir / f"trusttech_portable_hdd_{ts}.json"
