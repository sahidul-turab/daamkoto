"""
Trust Tech BD GPU scraper — custom Vue-based storefront.

Card: .product-card
Name: a.product-name (href for URL), a.product-name h6 (text)
Price: .product-card-bottom .product-price  (format: "BDT 9,000.00")
Stock: price presence
Pagination: ?page=N

Usage:
  python scrapers/trusttech/scrape_gpu.py           # print only
  python scrapers/trusttech/scrape_gpu.py --save    # save JSON to data/raw/
"""
...
START_URL = f"{BASE_URL}/categories/graphics-card"
PAGE_DELAY = 2.5
...
async def main(save: bool = False):
    ...
    if save:
        ...
        out_path = out_dir / f"trusttech_gpu_{timestamp}.json"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Scrape Trust Tech BD GPU listings")
