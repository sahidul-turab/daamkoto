"""
UCC RAM scraper
Extracts: name, price (BDT), stock, product URL from ucc.com.bd

Site structure (OpenCart):
- Card: .product-thumb
- Name: .caption .name a
- Price: .price .price-normal  (format: ৳84,000)
- Stock: .stats text contains "In Stock"
- Pagination: ?page=N

Usage:
  python scrapers/ucc/scrape_ram.py           # print only
  python scrapers/ucc/scrape_ram.py --save    # save JSON to data/raw/
"""
...
async def main(save: bool = False):
    ...
    if save:
        ...
        out_path = out_dir / f"ucc_ram_{timestamp}.json"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Scrape UCC RAM listings")
