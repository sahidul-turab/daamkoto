"""Pchouse Gaming Chair scraper — URL: /gaming-chair
PC House BD GPU scraper — custom OpenCart theme.
No desktop RAM category available on this site.

Card selector: .single-product-item
Price: .price-info .special-price (sale price), fallback .price-info .regular-price

Usage:
  python scrapers/pchouse/scrape_gpu.py           # print only
  python scrapers/pchouse/scrape_gpu.py --save    # save JSON to data/raw/
"""
...
START_URL = f"{BASE_URL}/gaming-chair"
PAGE_DELAY = 2.5
...
async def scrape_page(page, url: str) -> list[dict]:
    ...
    try:
        await page.wait_for_selector(".single-product-item", timeout=15_000)
    ...
    cards = await page.query_selector_all(".single-product-item")
    products = []
    scraped_at = datetime.now(timezone.utc).isoformat()

    for card in cards:
        name_el = await card.query_selector(".product-item-info h4 a")
        if not name_el:
        ...
        # Sale price preferred; fall back to regular price
        price_el = await card.query_selector(".price-info .special-price")
        if not price_el:
            price_el = await card.query_selector(".price-info .regular-price")
        raw_price = (await price_el.inner_text()).strip() if price_el else None
        price = clean_price(raw_price) if raw_price else None

        # Buy Now button present = in stock
...
async def main(save: bool = False):
    ...
    if save:
        ...
        out_path = out_dir / f"pchouse_gaming_chair_{timestamp}.json"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Scrape PC House BD gaming_chair listings")
