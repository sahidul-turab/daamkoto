"""
Ultra Technology BD RAM scraper
Extracts: name, price (BDT), stock, product URL from ultratech.com.bd

Site structure (OpenCart):
- Card: .product-thumb
- Name: .caption .name a
- Price: .price .price-new  (format: 2,500৳ — Taka symbol after number)
- Stock: no explicit indicator; price present = in stock
- Pagination: ?page=N

Usage:
  python scrapers/ultratech/scrape_ram.py           # print only
  python scrapers/ultratech/scrape_ram.py --save    # save JSON to data/raw/
"""
...
async def scrape_page(page, url: str) -> list[dict]:
    ...
    for card in cards:
        ...
        if not name_el:
            ...
            if name_el:
                name = (await name_el.get_attribute("alt") or "").strip()
            else:
                continue
        else:
...
async def main(save: bool = False):
    ...
    if save:
        ...
        out_path = out_dir / f"ultratech_ram_{timestamp}.json"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Scrape Ultra Technology RAM listings")
