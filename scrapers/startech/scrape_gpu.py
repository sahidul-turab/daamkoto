"""
StarTech GPU scraper
Extracts: name, price (BDT), product URL, inline specs from startech.com.bd/component/graphics-card

Same page structure as the RAM scraper (.p-item cards). Only the URL and output filename differ.

Usage:
  python scrapers/startech/scrape_gpu.py           # print only
  python scrapers/startech/scrape_gpu.py --save    # print + save JSON to data/raw/
"""
...
START_URL = f"{BASE_URL}/component/graphics-card"
PAGE_DELAY = 2.5
...
def save_results(products: list[dict]) -> Path:
    ...
    out_path = out_dir / f"startech_gpu_{timestamp}.json"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Scrape StarTech GPU listings")
