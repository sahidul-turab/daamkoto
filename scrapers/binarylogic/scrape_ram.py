"""
Binary Logic RAM scraper
Extracts: name, price (BDT), stock, product URL from binarylogic.com.bd

Site structure (StarTech fork):
- Card: .p-item
- Name: .p-item-name a
- Price: .p-item-price  (format: "৳ 13,900 ৳ 12,500" — take last/lowest price)
- Stock: price present = in stock
- Pagination: ?page=N

Usage:
  python scrapers/binarylogic/scrape_ram.py           # print only
  python scrapers/binarylogic/scrape_ram.py --save    # save JSON to data/raw/
"""
...
START_URL = f"{BASE_URL}/ram"
PAGE_DELAY = 2.5


def clean_price(raw: str) -> float | None:
    # May contain two prices: "৳ 13,900 ৳ 12,500" — take the last (current/sale)
    prices = re.findall(r"\d[\d,]+", raw)
    if not prices:
        return None
    last = prices[-1].replace(",", "")
    try:
...
async def scrape_page(page, url: str) -> list[dict]:
    ...
    for card in cards:
        ...
        buy_btn = await card.query_selector('button[type="submit"]')
...
async def main(save: bool = False):
    ...
    if save:
        ...
        out_path = out_dir / f"binarylogic_ram_{timestamp}.json"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Scrape Binary Logic RAM listings")
