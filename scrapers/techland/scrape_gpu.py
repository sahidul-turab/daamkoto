"""
Techland GPU scraper
Extracts: name, price (BDT), stock, product URL from techlandbd.com/pc-components/graphics-card

Same structure as scrape_ram.py — Tailwind CSS SPA with click-based pagination.

Usage:
  python scrapers/techland/scrape_gpu.py           # print only
  python scrapers/techland/scrape_gpu.py --save    # save JSON to data/raw/
"""
...
START_URL = f"{BASE_URL}/pc-components/graphics-card"
PAGE_DELAY = 2.5
...
async def main(save: bool = False):
    ...
    if save:
        ...
        out_path = out_dir / f"techland_gpu_{timestamp}.json"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Scrape Techland GPU listings")
