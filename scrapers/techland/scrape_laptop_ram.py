"""Techland Laptop RAM scraper — click-based SPA pagination. URL: /laptop-components/laptop-ram"""
...
START_URL = f"{BASE_URL}/laptop-components/laptop-ram"
PAGE_DELAY = 2.5
...
async def extract_products(page) -> list[dict]:
    ...
    for card in cards:
        ...
        products.append({"name": name, "price_bdt": price,
            ...
            "source": "Techland BD", "pc_bundle_only": pc_bundle_only, "scraped_at": scraped_at})
    return products

async def get_total_pages(page) -> int:
    return await page.evaluate("""() => {
        const btns = [...document.querySelectorAll('button')].filter(b => /^\\d+$/.test(b.innerText.trim()));
        const nums = btns.map(b => parseInt(b.innerText.trim(), 10));
        return nums.length > 0 ? Math.max(...nums) : 1;
    }""")

async def click_page(page, num: int) -> bool:
    return await page.evaluate("""(num) => {
        const btns = [...document.querySelectorAll('button')].filter(b => b.innerText.trim() === String(num));
...
async def main(save: bool = False):
    ...
    async with async_playwright() as pw:
        ...
        try: await page.wait_for_selector("article.products-list__item", timeout=15_000)
        ...
        for page_num in range(1, total_pages + 1):
            ...
            if page_num < total_pages:
                if not await click_page(page, page_num + 1): break
                await page.wait_for_timeout(int(PAGE_DELAY * 1000))
                try: await page.wait_for_selector("article.products-list__item", timeout=10_000)
                except PlaywrightTimeout: break
        await browser.close()
    if save:
        out_dir = Path("data/raw"); out_dir.mkdir(parents=True, exist_ok=True)
        ts = datetime.now().strftime("%Y%m%d_%H%M%S")
        out_path = out_dir / f"techland_laptop_ram_{ts}.json"
